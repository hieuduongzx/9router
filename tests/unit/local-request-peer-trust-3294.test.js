...
const { isLocalRequest, proxy } = await import("../../src/dashboardGuard.js");
...
describe("peer header trust", () => {
  ...
  it("keeps serving a genuinely local request stamped by the wrapper", async () => {
    const localRequest = request("/api/v1/models", {
      host: "localhost:20128",
      "x-9r-real-ip": "127.0.0.1",
      "x-9r-peer-token": PEER_TOKEN,
    });
    const response = await proxy(localRequest);
  ...
  it("accepts the legacy Host fallback only in development", async () => {
    ...
    const localRequest = request("/api/v1/models", { host: "localhost:20127" });
    const response = await proxy(localRequest);
