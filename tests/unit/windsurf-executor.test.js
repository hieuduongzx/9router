...
describe("WindsurfExecutor class", () => {
  it("uses the executor fallback while Windsurf stays hidden", () => {
  ...
  it("buildUrl returns the fallback GetChatMessage endpoint", () => {
    const ex = new WindsurfExecutor();
    expect(ex.buildUrl()).toBe("https://server.codeium.com/exa.language_server_pb.LanguageServerService/GetChatMessage");
  });

  it("keeps Windsurf hidden until tool calling is supported", () => {
