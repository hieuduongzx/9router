...
- **Models**: show effective input/output rates in the available-model catalog and let administrators save or restore per-model pricing overrides
- **i18n**: Thai (th) + Persian (fa) translations / README
- **Authentication**: replace the shared dashboard password with username/email accounts, open registration, admin/user roles, an administrator account manager, profile-menu sign out, and one-time CLI recovery passwords; the first registered account becomes administrator with no fixed default credentials
  - API keys and dashboard usage are isolated by account; legacy keys migrate to the primary administrator
  - User accounts are restricted to Home, their API keys, account-scoped usage, the available-model catalog, and account security; provider configuration and cross-account observability are administrator-only
  - Users can inspect request details generated by their own API keys; administrators get a system usage view with per-user attribution, active requests, and recent activity
  - Administrator-wide usage summaries and paginated request logs now share a single dedicated Activity view; Usage is the account-facing Overview and Details workspace
  - Activity logs attribute each request to its owning account and API key, with an administrator-only request detail drawer
  - Activity Logs support period filters and server-side pagination with 30 rows by default
  - Dashboard navigation now keeps Home global, groups shared tools under Account, and nests Media and System subsections inside the administrator-only Admin area
  - User accounts expose a persistent USD credit balance; administrators can add, deduct, or set exact cent balances from Accounts without allowing negative values
  - The product profile now combines balance, 30-day usage and API-key metrics, editable username/email with password re-authentication, password management, account metadata, and workspace links; the header account control shows the current balance, OIDC identities remain provider-managed, and system settings now live at `/dashboard/settings`
  - Usage Details and administrator Activity Logs show the estimated price of each request, including legacy request details recalculated from stored token usage
  - API keys use a trackable status list, and every `/v1`, `/api/v1`, `/v1beta`, and `/codex` request now requires an active key even on loopback
- **Remote access**: remove built-in Cloudflare Tunnel and Tailscale provisioning, status APIs, dashboard controls, and CLI setup; self-hosted endpoints remain local while managed deployments use the Cloud Endpoint

## Fixes
- **Database**: create all lightweight-backup tables before copying rows so foreign-key table order cannot break pre-migration backups
- **Database**: repair upgrades from schema v1 by creating the account table before API-key ownership migration
