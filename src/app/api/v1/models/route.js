...
export async function buildDashboardModelsList() {
  const [combos, publishedModels] = await Promise.all([
    getCombos(),
    getPublishedModels(),
  ]);
  return buildPublishedModelsCatalog(combos, publishedModels).map((model) => ({
    id: model.id,
    object: model.object,
...
export async function GET(request) {
  try {
    const data = await buildDashboardModelsList();
