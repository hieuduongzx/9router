...
const hasApi2KConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Api2K");
};

const getApi2KEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Api2K") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getApi2KEntry(config);
...
// POST - Apply Api2K config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Api2K entry or append
    const idx = config.findIndex((e) => e.name === "Api2K");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Api2K entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Api2K");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Api2K removed from Copilot config",
    });
  } catch (error) {
