...
export default function ModelCatalogPreview() {
  ...
  return (
    <section id="models" className="scroll-mt-16 border-y border-zinc-200 bg-zinc-50 px-5 py-20 sm:px-6 sm:py-24">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="overflow-hidden border border-zinc-200 bg-white">
          <div className="flex flex-col gap-3 border-b border-zinc-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 flex-1 gap-1 overflow-x-auto pb-1 sm:pb-0" aria-label="Filter models by provider">
              ...
              >
                All
                <span className={activeProvider === "all" ? "text-zinc-300" : "text-zinc-400"}>{models.length}</span>
              </button>
            ...
            <Link href="/models" className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-sm border border-zinc-200 px-3 font-mono text-xs font-semibold text-zinc-700 transition-colors hover:border-zinc-950 hover:text-zinc-950">
              Full catalog
          ...
          {loading && (
            <div className="p-4" aria-label="Loading model catalog">
              {Array.from({ length: 4 }).map((_, index) => (
                <div key={index} className="mb-2 h-14 animate-pulse bg-zinc-100 last:mb-0" />
              ))}
            </div>
          )}

          {!loading && error && (
            <div className="m-4 border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
            ...
            <>
                  ...
                  <article key={`mobile-${model.id}`} className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="break-all font-mono text-sm font-semibold text-zinc-950">{model.id}</p>
                        <p className="mt-1 text-sm text-zinc-500">{model.provider}</p>
                      </div>
                    </div>
                    <dl className="mt-4 grid grid-cols-3 gap-px border border-zinc-200 bg-zinc-200">
                      <div className="bg-zinc-50 p-3">
                        <dt className="text-xs text-zinc-500">Context</dt>
                        <dd className="mt-1 font-mono text-xs font-semibold text-zinc-950">{formatContext(model.capabilities?.contextWindow)}</dd>
                      </div>
                      <div className="bg-zinc-50 p-3">
                        <dt className="text-xs text-zinc-500">Input /M</dt>
                        <dd className="mt-1 font-mono text-xs font-semibold text-zinc-950">{formatRate(model.pricing?.input)}</dd>
                      </div>
                      <div className="bg-zinc-50 p-3">
                        <dt className="text-xs text-zinc-500">Output /M</dt>
                        <dd className="mt-1 font-mono text-xs font-semibold text-zinc-950">{formatRate(model.pricing?.output)}</dd>
                      </div>
                    </dl>
                  </article>
                ))}
              ...
              <div className="hidden overflow-x-auto lg:block">
                <table className="w-full min-w-[920px] border-collapse text-left">
                  <thead>
                    <tr className="border-b border-zinc-200 bg-zinc-50 font-mono text-[11px] font-semibold uppercase tracking-[0.08em] text-zinc-500">
                  ...
                  <tbody className="divide-y divide-zinc-200">
                    {visibleModels.map((model) => (
                      <tr key={model.id} className="transition-colors hover:bg-zinc-50">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="text-sm font-medium text-zinc-700">{model.provider}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4"><CapabilityList capabilities={model.capabilities} /></td>
                        <td className="px-5 py-4 text-right font-mono text-sm font-semibold text-zinc-700">
                          {formatContext(model.capabilities?.contextWindow)}
                        </td>
                        <td className="px-5 py-4 text-right font-mono text-sm font-semibold text-zinc-700">{formatRate(model.pricing?.input)}</td>
                        <td className="px-5 py-4 text-right font-mono text-sm font-semibold text-zinc-700">{formatRate(model.pricing?.output)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
