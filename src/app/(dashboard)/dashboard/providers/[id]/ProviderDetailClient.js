...
export default function ProviderDetailClient({ providerId, embedded = false, onClose }) {
  ...
  useEffect(() => {
    ...
    if (!isLiveCatalog) {
      // Reset provider-specific models when switching away.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setLiveModels([]);
      return;
    }
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <Card>
        ...
        {!!modelsTestError && (
          <div className="mb-3">
            <p className="text-xs text-destructive break-words">{modelsTestError}</p>
        ...
        {providerId === "zed" && !!liveModelsError && (
          <p className="text-xs text-destructive mb-3 break-words">{liveModelsError}</p>
        )}
        {renderModelsSection()}
      </Card>
...
ProviderDetailClient.propTypes = {
