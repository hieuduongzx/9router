...
export default function ApiKeySelect({ value, onChange, apiKeys = [], cloudEnabled = false, className = "" }) {
  ...
  if (noKeys) {
    return (
      <span className={`min-w-0 rounded bg-surface/40 px-2 py-2 font-mono text-xs text-muted-foreground sm:py-1.5 ${className}`}>
    ...
    <div className={`flex flex-col gap-1.5 ${className}`}>
      <div className="flex items-center gap-2">
        <select
          ...
          className="flex-1 min-w-0 px-2 py-2 bg-surface rounded font-mono text-xs border border-border focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5"
        >
          {options.map((o) => (
        ...
        {isSaved && (
          <button
            ...
            className="p-1 text-muted-foreground hover:text-destructive rounded-sm transition-colors shrink-0"
