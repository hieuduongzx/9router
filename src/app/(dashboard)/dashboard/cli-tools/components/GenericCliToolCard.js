...
export default function GenericCliToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (currentBaseUrl && matchKnownEndpoint(currentBaseUrl)) {
      return "configured";
    }
    return "configured";
  };
                      ...
                      <div className="flex flex-wrap gap-1.5 min-h-[36px] p-2 bg-surface rounded border border-border">
                        {selectedModels.length === 0 ? (
                          ...
                          selectedModels.map((modelId) => (
                            <span
                              key={modelId}
                              className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-secondary text-xs text-text-main border border-border"
                            >
                              <span>{modelId}</span>
                              <button
                                type="button"
