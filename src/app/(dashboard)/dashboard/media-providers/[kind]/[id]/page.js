...
export default function MediaProviderDetailPage() {
  ...
  const matchesKind = kind === "audio"
    ...
    : kinds.includes(kind);
  if (!isCustom && !matchesKind) return notFound();
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Models - hidden for audio/tts/webSearch/webFetch (provider IS the model); custom uses prefix as alias */}
      {kind !== "audio" && kind !== "tts" && kind !== "webSearch" && kind !== "webFetch" && (
        <ModelsCard
          providerId={id}
          kindFilter={kind}
