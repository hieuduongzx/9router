...
function RouterNode({ data }) {
  return (
    <div className="flex items-center justify-center px-5 py-3 rounded-xl border-2 border-primary bg-primary/5 shadow-md min-w-[130px]">
      ...
      <img src="/favicon.svg" alt="Api2K" className="w-6 h-6 mr-2" />
      <span className="text-sm font-bold text-primary">Api2K</span>
      {data.activeCount > 0 && (
        <span className="ml-2 px-1.5 py-0.5 rounded-full bg-primary text-primary-foreground text-xs font-bold">
          {data.activeCount}
        </span>
      )}
    </div>
