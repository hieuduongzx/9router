...
export default function SettingsPage() {
    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        setSamlForm({
          ...
          samlIssuer: data?.samlIssuer || "urn:router2k:sp",
    ...
    reader.onload = (e) => {
      try {
        ...
        setSamlForm((prev) => ({
          ...
          samlIssuer: prev.samlIssuer || "urn:router2k:sp",
  ...
  const saveSamlSettings = async (targetAuthMode = oidcForm.authMode || "password") => {
    ...
    try {
      const payload = {
        ...
        samlIssuer: samlForm.samlIssuer.trim() || "urn:router2k:sp",
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Router2k-backup-${stamp}.json`;
              ...
              <p className="text-xs text-text-muted">
                {settings.authMode === "sso" || settings.authMode === "oidc" || settings.authMode === "saml"
                  ...
                  : settings.authMode === "both"
                    ? `Accounts + ${settings.ssoType === "saml" ? "SAML 2.0" : "OIDC"} active`
                  ...
                  {[
                    {
                      value: "password",
                      title: "Accounts only",
                      desc: "Use username or email with a password.",
                    },
                    {
                      value: "sso",
                    ...
                    },
                    {
                      value: "both",
                      title: "Both",
                      desc: "Allow an account or SSO login.",
                    },
                  ].map((option) => {
                        ...
                        <div className="p-2.5 rounded border border-primary/20 bg-primary/5 text-primary text-xs">
                          ...
                          <ul className="list-disc pl-4 space-y-1 font-mono text-[11px]">
                            ...
                            <li>
                              ...
                              <code className="bg-bg px-1 py-0.5 rounded break-all">{samlForm.samlIssuer || "urn:router2k:sp"}</code>
                            </li>
                            <li>
                              <b>NameID Format:</b>{" "}
