...
export const CLI_TOOLS = {
  ...
  cursor: {
    ...
    notes: [
      ...
      { type: "cloudCheck", text: "Cursor routes requests through its own server, so a local endpoint is not supported. Enable the Cloud Endpoint in Settings." },
    ],
    guideSteps: [
  ...
  amp: {
    ...
    notes: [
      { type: "info", text: "Use Router2k model aliases to keep Amp shorthand mappings stable across provider updates." },
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to Router2k aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via Router2k",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. Router2k works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in Router2k can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use Router2k with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. Router2k will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. Router2k writes a [model.9router] custom model and sets it as the default.",
      },
      {
        type: "info",
